...
// Wersja 1 do 12.09.2020 - Nie wysyłała z planety transportu gdy nie było MT.
/*
    @Override
    public void execute()
    {
        if (isRun())
        {
            if (isSleepTimeOut(System.currentTimeMillis()))
            {
                int metal = 0, krysztal = 0, deuter = 0, sumaSurowcow = 0;
                ...
                for(AutotransportInfo autotransportInfo : AutotransportTabController.getAutoTransportInfoList())
                {
                    ...
                    app.data.autotransport.Autotransport autotransport = autotransportInfo.getAutotransport();
                    //Sprawdzenie czy wysyłanie surowców z planety jest aktywne
                    if(autotransport.isWyslijUser() &&  autotransport.isWyslijPonownie() && dostepneWolneFloty())
                    {
                        //Sprawdzenie czy wybrana jest prawidłowa planeta, jeżeli nie to kliknięcie w planetę.
//                        while(ListaPlanet.wybranaPlaneta(OgameWeb.webDriver) != p.getId())
//                        {
//                            p.clickPlanet();
...
//                        }
                        ...
                        if(dostepneWolneFloty())
                        {
                            ...
                            if(sumaSurowcow > MIN_ILOSC_SUROWCOW)
                            {
                                ...
                                {
                                    //Sprawdzenie czy wystarczy samych MT czy nalezy wysłać DT
                                    if(potrzebnaIloscMT <= iloscMTZadeklarowana )
                                        potrzebnaIloscDT = 0;
                                    else
                                    {
                                        int tmp = sumaSurowcow;
                                ...
                                {
                                    //Sprawdzenie czy wystarczy samych DT czy nalezy wysłać MT
                                    if(potrzebnaIloscDT <= iloscDTZadeklarowana )
                                        potrzebnaIloscMT = 0;
                                    else
                                    {
                                        int tmp = sumaSurowcow;
